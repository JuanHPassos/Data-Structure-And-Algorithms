/*
- Deve ser elaborada uma função rpn de acordo com a declaração presente no arquivo 
Pilha.h (com o mesmo nome, entrada e saída).

```c
float rpn(char *sequencia);
```

- A solução para o problema deve ser elaborado em um arquivo .c chamado rpn.c
- O arquivo que será submetido ao RunCodes deve ser um .zip apenas com a solução rpn.
*/
